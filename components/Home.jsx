...
export default function Homepage() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      loadHistory();
    } catch (err) { alert("Server busy. Please try again."); }
    setLoading(false);
  };
  ...
  return (
    <div className="flex min-h-screen bg-[#FDFDFF] text-slate-900 font-sans">
      {/* Premium Sidebar */}
      <aside className="w-80 bg-white border-r border-slate-100 p-8 hidden xl:flex flex-col shadow-sm">
        <div className="flex items-center gap-3 mb-12">
          ...
          <h2 className="text-xl font-black tracking-tighter uppercase italic text-slate-800">Generator</h2>
        </div>
        
        <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-6">Recent Sessions</h3>
        <div className="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar">
          {history.map(h => (
            <div key={h._id} className="group flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-50 hover:border-indigo-100 hover:bg-white transition-all duration-200">
              ...
              <button 
                ...
                className="opacity-0 group-hover:opacity-100 ml-2 p-2 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-all"
              >
      ...
      <main className="flex-1 overflow-y-auto px-6 py-12 lg:px-16">
        <div className="max-w-4xl mx-auto">
          <header className="mb-12 text-center lg:text-left">
            <h1 className="text-5xl font-black tracking-tight mb-4 text-slate-900 leading-tight">
              AI <span className="text-indigo-600">Question</span> Studio
            </h1>
            <p className="text-slate-500 text-lg font-medium">Assignment: Web Service Deployment & Operations</p>
          </header>

          {/* Interactive Upload Card */}
          <section className="bg-white rounded-[2.5rem] p-8 md:p-10 shadow-2xl shadow-slate-200/60 border border-slate-50 mb-16 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-50 rounded-full -mr-16 -mt-16 opacity-50"></div>
            
            <div className="flex flex-col md:flex-row items-stretch md:items-end gap-8 relative z-10">
              <div className="w-full md:w-40">
                <label className="block text-[11px] font-black text-slate-400 uppercase tracking-widest mb-4 ml-1">Quantity</label>
                <input 
              ...
              <div className="flex-1 w-full relative">
                ...
                <label 
                  ...
                  className={`flex items-center justify-center gap-4 w-full h-[72px] rounded-2xl font-black text-lg transition-all cursor-pointer shadow-xl active:scale-[0.98] ${
                    loading ? 'bg-indigo-100 text-indigo-400 cursor-wait animate-pulse' : 'bg-indigo-600 text-white shadow-indigo-200 hover:bg-indigo-700 hover:shadow-indigo-300'
                  }`}
                >
                   {loading ? "ANALYZING PDF..." : "UPLOAD PDF & GENERATE"}
                </label>
              </div>
            </div>
            <p className="text-[11px] text-slate-400 mt-6 text-center font-bold tracking-tight">
               SYSTEM OPTIMIZATION: ONLY THE FIRST 2 PAGES ARE PROCESSED TO ENSURE FAST RESPONSE.
            </p>
          </section>
            ...
            <div className="space-y-6">
              {questions.length === 0 && !loading && (
                <div className="py-24 text-center bg-slate-50/50 border-2 border-dashed border-slate-100 rounded-[3rem]">
                  <p className="text-slate-300 font-bold italic">No questions active. Start by uploading a document.</p>
                </div>
              )}
              {questions.map((q, i) => (
